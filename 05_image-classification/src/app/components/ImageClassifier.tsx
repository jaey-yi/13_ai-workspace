...
export default function ImageClassifier() {
  ...
  useEffect(() => {
    ...
    async function loadModel() {
      ...
      try {
        ...
        setError(
          error instanceof Error ? error.message : "모델 로딩 중 오류 발생",
        );
        if (progressInterval) clearInterval(progressInterval);
      } finally {
  ...
  }, []); //빈배열의 뜻? 의존성 배제하기 위해? 최초 1회 실행을 위해
  ...
  const classifyImage = async () => {
    ...
    try {
      const results = await model.classify(imgElement, 3);
      setPredictions(results);
    } catch (error) {
      setError(error instanceof Error ? error.message : "분류실피");
    } finally {
